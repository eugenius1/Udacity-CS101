...
def is_colluded(origin, dest, graph, k):
    v=0
    if k>=0 and origin==dest:
        return True
    current_pages = [origin]
    prev_pages = []
    for unused in range(0, k):
        for page in current_pages:
            if dest in graph[page]:
                if v: print origin," colluded by ",unused," with ",dest
                return True
        prev_pages += current_pages
        current_pages = []     # origin not found in this collusion level
        for page in prev_pages:
            current_pages += graph[page]
            # OR for link in graph[page]: if link not in prev_pages: current_pages.append(link)
    if v: print origin," colluded by >",k," with ",dest
    return False
...
def compute_ranks(graph, k):
    ...
    for i in range(0, numloops):
        ...
        for page in graph:
            newrank = (1 - d) / npages
            for node in graph:
                if page in graph[node] and not is_colluded(page, node, graph, k):
...
print compute_ranks(g, 0) == {'a': 0.26676872354238684, 'c': 0.1216391112164609, 'b': 0.1216391112164609, 'd': 0.1476647842238683}

# a->a, a->b, b->a links are reciprocal
print compute_ranks(g, 1) == {'a': 0.14761759762962962, 'c': 0.08936469270123457, 'b': 0.04999999999999999, 'd': 0.12202199703703702}

# a->a, a->b, b->a, a->c, c->d, d->a links are reciprocal
# (so all pages end up with the same rank)
print compute_ranks(g, 2) == {'a': 0.04999999999999999, 'c': 0.04999999999999999, 'b': 0.04999999999999999, 'd': 0.04999999999999999}
